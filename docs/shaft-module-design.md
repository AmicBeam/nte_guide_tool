...
- 单次倾陷伤害由四名角色分别贡献。每名角色使用其整轴动作快照的平均倾陷增伤、平均穿防、平均减防和平均减抗计算，再乘以倾陷频率计入全队总伤与 DPS；平均倾陷增伤按动作快照将“倾陷强度 ÷ 300”与明确的倾陷伤害增伤相加后取平均，当前包含达芙蒂尔「分光」 +20% 和残红觉醒的全队 +300%。倾陷贡献不计入角色自身伤害、角色自身占比或动作贡献。配装页在角色贡献列表后使用同款行式布局提供倾陷占比与“分析详情”，弹窗展示各角色倾陷贡献、占倾陷比例、单次贡献与平均参数。达芙蒂尔「洞察」额外造成的倾陷伤害作为独立次数倍率结算，不计入“平均倾陷增伤”展示。
