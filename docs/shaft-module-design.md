...
- 九原天赋一按用户 2026-07-19 口径作为创生与创生复制体的独立频率乘区 ×2；娜娜莉按用户 2026-07-21 口径只改写原创生花数量，不影响伊洛伊固定20朵的创生复制体。该频率乘区仍分别作用于二者，并保持单次伤害和各自原持续窗口不变。Nanoka 原始描述为“额外生成1株创生株”，该频率折算属于项目实现口径。分析详情统一将创生、创生复制体和浊燃使用的 `frequency_multiplier` 显示为“频率乘区”；DOT（包括浊燃）的期望暴击乘区统一显示为“双暴区”。
