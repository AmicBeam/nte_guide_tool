...
- 残红在2026-08-04以未公开测试角色接入。Nanoka 当前未收录该角色；面板与弧盘来自用户维护的腾讯文档，动作数值、技能描述和觉醒描述以用户2026-08-06提供的 `SkillDamageData_Zankou_Radio.xlsx` 为准，不表述为 Nanoka 已核实数据。1级攻击/生命/防御为83 / 1280 / 75，80级为660 / 15513 / 909；排轴基础面板使用80级数值，适应类型为气态。表内1级倍率、段数、终结能量累积、环合值和倾陷值按倍率呈现方式聚合进入动作数据。「狩」「惑」「蚀心」「鸩火」、幻a对全部现存DOT加层、天赋二环合强度、A–F六个觉醒和三觉/六觉共鸣均进入计算与自检。A觉扩散按动作一次对原目标的每种已有DOT加1层，并使狩/惑永久提升至40%/50%；B觉按残红作为环合任一方参与浊燃/覆纹积累最多2层蓄焰，焚天消费并按每层最终伤害+100%；C觉把抗打断+50%投影到动作面板，并以独立回复事件每秒结算4%最大生命；D觉以终结能量蓄满作为血宴自动激活条件，血宴享受蓄焰且不消费；E觉仅在非循环单敌轴记录首次强制倾陷，同时全队倾陷伤害+300%；F觉读取幻e末段前目标DOT总层数，仅按最后一段倍率折算每层+12%、最多20层。当前多目标不模拟，涉及逐目标的A/E/F按单敌目标等价结算。点亮任意3个A–F节点触发三觉共鸣，其中援护「饲火」不获得技能等级+1；点亮全部6个节点触发六觉共鸣，依赖击败浊燃目标的效果按用户2026-08-05口径忽略。e、强化e、幻e共享5秒CD并使用同一冷却键参与自检。残红天赋一的全队DOT施加触发仍为缺口。
